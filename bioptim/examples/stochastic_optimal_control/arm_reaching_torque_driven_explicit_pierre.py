...
from bioptim.interfaces.stochastic_biorbd_model import StochasticBiorbdModel
...
def stochastic_forward_dynamics(
    ...
    dqdot_constraint = nlp.model.stochastic_dynamics(q, qdot, tau, ref, k, with_gains=with_gains)
    defects = cas.vertcat(dqdot_constraint - qddot)

    return DynamicsEvaluation(dxdt=cas.vertcat(qdot, qddot, qdddot), defects=defects)
...
def get_cov_mat(nlp, node_index):
    ...
    motor_noise = nlp.model.motor_noise
    sensory_noise = nlp.model.sensory_noise
    ...
    nlp.model.stochastic_jacobian(
        q=nlp.states["q"].cx_start,
        qdot=nlp.states["qdot"].cx_start,
        tau=nlp.controls["tau"].cx_start,
        ref=nlp.stochastic_variables["ref"].cx_start,
        k=nlp.stochastic_variables["k"].cx_start,
    )

    ddx_dwm = vertcat(MX.zeros(n_qdot, n_q + n_qdot), qddot_jac, MX.zeros(n_qddot, n_q + n_qdot))
...
def prepare_socp(
    ...
    bio_model = StochasticBiorbdModel(
        bio_model,
        sensory_noise=sensory_noise_magnitude,
        motor_noise=motor_noise_magnitude,
        force_field=force_field_magnitude,
    )
